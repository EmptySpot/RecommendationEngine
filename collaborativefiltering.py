...
bufcursor = mySQLConnection.cursor(buffered=True)
...
def profielbekijker(i):
    sql_selecter = "SELECT prodid FROM profiles_previously_viewed WHERE profid = %s"
    tering = (i[1],)
    cursor.execute(sql_selecter,(tering))
...
def nogmeerrec(nieuwequery, run, i, recommendaties):
    ...
    bufcursor.execute(mySql_query, (nieuwequery))
    replyervan = bufcursor.fetchmany(hoeveelheid)
    run = 1
...
def collabrec():
    bufcursor.execute("SELECT profid, kindofcust, recommendsforitem1, recommendsforitem2 FROM profielgezien")
    colabres = bufcursor.fetchmany(hoeveelheid)
    for i in colabres:
        ...
        bufcursor.execute(mySql_query, (tuple(querylist)))
        replyervan = bufcursor.fetchmany(hoeveelheid)
        recommendaties = []
        run = 0
        recommendatiesmaken(replyervan, run, i, recommendaties)
...
def segment():
    gebruikteids = []
    bufcursor.execute("SELECT segment, profid FROM sessions WHERE segment NOT IN ('Bouncer', 'Leaver')")
    resultaatseg = bufcursor.fetchmany(hoeveelheid)
